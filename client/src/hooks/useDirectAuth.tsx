...
// EMERGENCY FIX: clear ALL localStorage on page load to ensure we don't keep getting admin data
if (typeof window !== 'undefined') {
  console.log('EMERGENCY FIX: Clearing all authentication data');
...
export const DirectAuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Check if user is logged in - ALWAYS query the server directly
  useEffect(() => {
    const checkAuth = async () => {
      setIsLoading(true);
      
      // JUST USE SERVER - no more localStorage for initial check
      try {
        console.log('Directly querying server for auth status');
        const response = await axios.get('/api/direct-user');
        
        if (response.data.success && response.data.user) {
          console.log('Authentication successful via direct API:', response.data.user);
          ...
          console.log('Not authenticated via direct API');
          setUser(null);
          localStorage.removeItem('user');
          localStorage.removeItem('isAuthenticated');
        }
